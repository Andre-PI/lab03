...
print("\nPesos de atenção após máscara (linha=token atual, coluna=token atendido):")
print(np.round(attn_weights, 4))
print("\n→ Triângulo superior = 0.0: token i não vê token i+1 ✓")

print("\n" + "=" * 50)
print("TAREFA 2 — Cross-Attention Encoder-Decoder")
print("=" * 50)
...
print(f"Shape saída cross-att: {cross_out.shape}")
print(f"\nPesos cross-attention (4 tokens do decoder × 10 tokens do encoder):")
print(np.round(cross_weights[0], 3))
print("\n→ Todos os 10 tokens do encoder são visíveis (sem máscara) ✓")
...
while step < max_tokens:
    ...
    if next_word == "<EOS>":
        print("\n→ Token <EOS> encontrado. Geração encerrada.")
        break
